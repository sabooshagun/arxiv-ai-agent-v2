...
SECTOR_KEYWORDS: Dict[str, List[str]] = {
    "Education": [
        "education", "edtech", "student", "students", "teacher", "teachers", "classroom",
        "curriculum", "learning analytics", "assessment", "tutoring", "mooc"
    ],
    "Healthcare": [
        "healthcare", "health care", "clinical", "clinician", "hospital", "patient", "patients",
        "medical", "medicine", "diagnosis", "treatment", "ehr", "electronic health record",
        "radiology", "telemedicine"
    ],
    "Finance": [
        "finance", "financial", "bank", "banking", "loan", "credit", "fraud", "risk",
        "trading", "portfolio", "investment", "fintech", "insurance", "actuarial"
    ],
    "Human Resources": [
        "human resources", "hr", "recruiting", "recruitment", "hiring", "talent", "workforce",
        "employee", "employees", "performance review", "attrition", "payroll"
    ],
    "Life Sciences": [
        "life science", "biology", "biological", "bioinformatics", "protein", "drug",
        "pharma", "pharmaceutical", "clinical trial", "molecular", "cell", "omics"
    ],
    "Biotech": [
        "biotech", "biotechnology", "synthetic biology", "lab automation", "wet lab",
        "high-throughput", "sequencing"
    ],
    "Retail": [
        "retail", "store", "stores", "merchandising", "inventory", "assortment",
        "pricing", "promotion", "point of sale", "pos"
    ],
    "E-commerce": [
        "e-commerce", "ecommerce", "online shopping", "marketplace", "shopping cart", "product catalog"
    ],
    "Marketing": [
        "marketing", "advertising", "ads", "campaign", "attribution", "conversion", "crm",
        "brand", "segmentation"
    ],
    "Supply Chain & Logistics": [
        "supply chain", "logistics", "warehouse", "fulfillment", "inventory management",
        "route planning", "last-mile", "shipping", "delivery"
    ],
    "Manufacturing": [
        "manufacturing", "factory", "production line", "quality inspection",
        "predictive maintenance", "industrial", "process control"
    ],
    "Energy": [
        "energy", "power grid", "electricity", "renewable", "solar", "wind",
        "battery", "oil", "gas", "smart grid"
    ],
    "Transportation": [
        "transportation", "traffic", "mobility", "autonomous driving", "self-driving",
        "vehicle", "aviation", "rail"
    ],
    "Telecom": [
        "telecom", "telecommunications", "5g", "network slicing", "cellular", "base station"
    ],
    "Cybersecurity": [
    ...
    "Government & Public Sector": [
        "government", "public sector", "policy", "civic", "public service", "smart city"
    ],
    "Legal": [
        "legal", "law", "contract", "litigation", "case law", "compliance", "regulation"
    ],
    "Real Estate": [
        "real estate", "property", "housing", "mortgage", "rent", "tenant", "landlord"
    ],
    "Media & Entertainment": [
        "media", "news", "journalism", "music", "movie", "film", "entertainment", "streaming"
    ],
    "Gaming": [
        "game", "gaming", "esports", "player modeling", "matchmaking"
    ],
    "Social Platforms": [
        "social network", "social media", "ugc", "user generated content",
        "community", "creator"
    ],
    "Agriculture": [
    ...
    "Climate & Environment": [
        "climate", "carbon", "emission", "sustainability", "environment", "pollution", "ecosystem"
    ],
    "Robotics": [
        "robot", "robotics", "manipulation", "grasping", "navigation", "slam"
    ],
}


def _paper_text_for_sector(p: Paper) -> str:
    return f"{p.title or ''} {p.abstract or ''}".lower()


def filter_papers_by_sector(papers: List[Paper], sector_filter: str) -> List[Paper]:
    if not papers:
        return papers

    if not sector_filter or sector_filter == "All Sectors":
        return papers

    keywords = SECTOR_KEYWORDS.get(sector_filter, [])
    if not keywords:
        return papers

    filtered: List[Paper] = []
    for p in papers:
        text = _paper_text_for_sector(p)
...
def main():
    ...
    with st.sidebar:
        ...
        sector_filter = st.selectbox(
            "Filter by sector",
            ["All Sectors"] + sorted(SECTOR_KEYWORDS.keys()),
            index=0,
            help="Optional: keyword match on title/abstract (simple heuristic)."
        )
        # Persist the user's sector selection so later stages can read it from session_state
...
# Save the venue settings for later use after embeddings.
    ...
    # Apply sector filtering AFTER embeddings (optional)
    sector_filter = st.session_state.get("sector_filter", "All Sectors")

    before_s = len(candidates)
    candidates = filter_papers_by_sector(candidates, sector_filter)
    after_s = len(candidates)

    if sector_filter and sector_filter != "All Sectors":
        st.info(
            f"Sector filter `{sector_filter}` applied. "
            f"Remaining: {after_s} (Filtered out {before_s - after_s})"
        )
